...
const { fileExist, readFileByLine } = rewiremock.proxy(() => require('../../src/utils/file'), () => mock)
...
describe('File', function () {
  ...
  describe('fileExist', () => {
    it('Should return true if the file exists', () => {
      ...
      assert.strictEqual(fileExist('file.txt'), true)
    })

    it('Should return false if the file exists ', () => {
      fsStub.existsSync.returns(false)
      assert.strictEqual(fileExist('file.txt'), false)
    })
  })
  ...
  describe('readFileByLine', () => {
    ...
    it('Should return a string list of all lines in the file ', async () => {
      fsStub.existsSync.returns(true)
      const readFilePromise = readFileByLine('filePath')
